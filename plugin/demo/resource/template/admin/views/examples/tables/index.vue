    ...
    <ElCard shadow="never" class="art-card-xs">
      ...
      <div>
        <p class="m-0 mb-4 leading-[1.6] text-g-700">
          集成搜索、刷新、全屏、大小控制、列显示隐藏、拖拽排序、表格样式控制、并内置
          useTable 组合式函数，提供强大的组合式 API，集成数据获取、智能缓存（LRU算法）、
...
import { fetchGetUserList } from "@/apps/demo/api/system-manage";
import { ACCOUNT_TABLE_DATA } from "@/apps/demo/mock/temp/formData";
...
const getCacheKeySummary = (key: string): string => {
  try {
    ...
    return `页码: ${params.current || 1}, 大小: ${params.size || 20}${
      params.name ? ", 名称: " + params.name : ""
    }${params.status ? ", 状态: " + params.status : ""}`;
  } catch {
    return "无效的缓存键";
  }
};
  ...
  switch (command) {
    ...
    case "batchUpdateColumns": {
      ...
      updateColumn?.([
        {
          prop: "userGender",
          updates: { width: 200, label: "性别-已更新", sortable: false },
        },
        {
          prop: "userPhone",
          updates: { width: 200, label: "手机号-已更新", sortable: false },
        },
      ]);
