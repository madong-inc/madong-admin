...
abstract class BaseDao
{
     ...
     * @throws \Exception
     */
    public function selectModel(array $where, string $field = '*', int $page = 0, int $limit = 0, string $order = '', array $with = [], bool $search = false): \Illuminate\Database\Eloquent\Builder|\Illuminate\Database\Query\Builder|\Illuminate\Pagination\LengthAwarePaginator|null
    {
        // 获取模型的查询构建器
        $query = $this->getModel()->query();
        // 根据是否需要搜索来决定查询条件
     ...
     * @throws \Exception
     */
    public function get($id, ?array $field = null, ?array $with = [], string $order = ''): ?\Illuminate\Database\Eloquent\Model
    {
        // 构建查询条件
