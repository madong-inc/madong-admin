...
import GrantTenantWrap from "./modules/grant-tenant-warp.vue";
 ...
 * @param param0
 */
function onActionClick({ code, row }: OnActionClickParams<TenantMemberRow>) {
  switch (code) {
    case "delete": {
      onDelete(row);
      break;
...
const [Grid, gridApi] = useVxeGrid({
  ...
  gridOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues = {} as TenantMemberRow) => {
...
const [UserTenantDrawer, userTenantApi] = useDrawer({
  connectedComponent: GrantTenantWrap,
  destroyOnClose: true,
});
...
async function onEdit(record: TenantMemberRow) {
  drawerApi.setData({ ...record, isUpdate: true });
  drawerApi.open();
}

async function onDelete(row: TenantMemberRow) {
 ...
 * @param row
 */
async function onLocked(row:TenantMemberRow){
    await confirm(
 ...
 * @param row
 */
async function onUnLocked(row:TenantMemberRow){
  await confirm(
 ...
 * @param row
 */
async function onResetPassword(row:TenantMemberRow){
  let ids = [];
  if (row) {
    ids.push(row.id);
  } else {
...
function onMultiDelete() {
  ...
  const ids = rows.map((row: TenantMemberRow) => row.id);
  Modal.confirm({
 ...
 * @param row 
 */
function onUserTenant(row:TenantMemberRow){
  userTenantApi.setData({record:row}).open();
}
...
<template>
  <Page :auto-content-height="true" content-class="flex gap-[8px] w-full">
    ...
    <TenantGrantModal @reload="gridApi.query()" />
    <UserTenantDrawer @reload="gridApi.query()"/>
  </Page>
</template>
