...
import { GatewayBlacklistApi } from "#/api/dev/gateway";
...
const [Grid, gridApi] = useVxeGrid({
  ...
  gridOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues = {} as GatewayBlacklistRow) => {
 ...
 * 表格操作按钮的回调函数
 */
function onActionClick({ code, row }: OnActionClickParams<GatewayBlacklistRow>) {
  switch (code) {
    case "edit": {
      handleEdit(row);
      break;
 ...
 */
 async function onStatusChange(
  newStatus: number,
  row: GatewayBlacklistRow,
) {
  const status: Recordable<string> = {
    0: $t('dev.blacklist.list.table.columns.actions.status.disabled'),
 ...
 * @param record
 */
const handleEdit = (record: GatewayBlacklistRow) => {
...
// 删除
function handleDelete(row: GatewayBlacklistRow) {
...
async function handleMultiDelete(){
  ...
  const ids = rows.map((row: GatewayBlacklistRow) => row.id);
  await confirm(
    $t("dev.blacklist.list.table.toolbar.delete.confirm", [ids.length]),
    $t("dev.blacklist.list.table.toolbar.delete.title")
  );
    ...
    <Grid>
      <template #toolbar-buttons>
        <TableAction
          :actions="[
            {
              label: $t('dev.blacklist.list.table.toolbar.create'),
              type: 'primary',
            ...
            {
              label: $t('dev.blacklist.list.table.toolbar.delete.label'),
              type: 'primary',
