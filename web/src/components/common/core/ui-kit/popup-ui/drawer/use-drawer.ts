...
import DrawerInject from './drawer.vue';

const USER_DRAWER_INJECT_KEY = Symbol('MADONG_DRAWER_INJECT');
...
>(options: DrawerApiOptions = {}) {
  ...
  const Drawer = defineComponent(
    (props: DrawerProps, { attrs, slots }) => {
      return () =>
        h(DrawerInject, { ...props, ...attrs, drawerApi: extendedApi }, slots);
    },
    {
      inheritAttrs: false,
      name: 'Drawer',
...
async function checkProps(api: ExtendedDrawerApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      ...
      console.warn(
        `[Drawer]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Drawer, please use useDrawer or api.`,
      );
    }
  }
}
