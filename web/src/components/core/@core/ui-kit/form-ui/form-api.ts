...
import type { FormActions, FormSchema, BasicFormProps} from './types';
...
function getDefaultState(): BasicFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  ...
  public state: null | BasicFormProps = null;
  stateHandler: StateHandler;

  public store: Store<BasicFormProps>;
  ...
  private prevState: null | BasicFormProps = null;

  constructor(options: BasicFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<BasicFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: BasicFormProps) => Partial<BasicFormProps>)
      | Partial<BasicFormProps>,
  ) {
