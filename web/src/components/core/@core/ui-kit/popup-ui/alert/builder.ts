...
import type {Recordable} from '#/components/core/@core/typings';

import type {AlertProps, BeforeCloseScope, PromptProps} from './alert';

import {useSimpleLocale} from '#/components/core/effects/hooks';
import {Input, BasicRenderContent} from '#/components/core/@core/ui-kit/shadcn-ui';
import {isFunction, isString} from '#/components/core/utils';
...
export function confirm(options: AlertProps): Promise<void>;
...
export function confirm(
    ...
    if (!arg1) {
        return isString(arg0)
            ? alert(arg0, defaultProps)
            : alert({...defaultProps, ...arg0});
    } else if (!arg2) {
        return isString(arg1)
            ? alert(arg0 as string, arg1, defaultProps)
            : alert(arg0 as string, {...defaultProps, ...arg1});
    }
    return alert(arg0 as string, arg1 as string, {
        ...defaultProps,
        ...arg2,
    });
}
...
export async function prompt<T = any>(
    ...
    staticContents.push(h(BasicRenderContent, { content, renderBr: true }));
