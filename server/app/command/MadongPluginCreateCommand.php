...
class MadongPluginCreateCommand extends Command
{
    protected static string $defaultName = 'madong-plugin:create';
    protected static string $defaultDescription = 'Madong App Plugin Create';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (is_dir($plugin_config_path = base_path()."/madong/plugins/$name")) {
    ...
    {
        $base_path = base_path();
        $this->mkdir("$base_path/madong/plugins/$name/app/controller", 0777, true);
        $this->mkdir("$base_path/madong/plugins/$name/app/model", 0777, true);
        $this->mkdir("$base_path/madong/plugins/$name/app/middleware", 0777, true);
        $this->mkdir("$base_path/madong/plugins/$name/app/view/index", 0777, true);
        $this->mkdir("$base_path/madong/plugins/$name/config", 0777, true);
        $this->mkdir("$base_path/madong/plugins/$name/public", 0777, true);
        $this->mkdir("$base_path/madong/plugins/$name/api", 0777, true);
        $this->createFunctionsFile("$base_path/madong/plugins/$name/app/functions.php");
        $this->createControllerFile("$base_path/madong/plugins/$name/app/controller/IndexController.php", $name);
        $this->createViewFile("$base_path/madong/plugins/$name/app/view/index/index.html");
        $this->createConfigFiles("$base_path/madong/plugins/$name/config", $name);
        $this->createApiFiles("$base_path/madong/plugins/$name/api", $name);
        $this->createInstallSqlFile("$base_path/madong/plugins/$name/install.sql");
    }
...
namespace madong\\plugins\\$name\\app\\controller;
...
namespace madong\\plugins\\$name\api;

use madong\services\Menu;
use support\Db;
use Throwable;
...
return [
    'files' => [
        base_path() . '/madong/plugins/$name/app/functions.php',
    ]
];
EOF;
...
return [
    ...
    'path' => base_path() . "/madong/plugins/$name/resource/translations",
];
